...
public class SHA1 {
    ...
    private int process_input_bytes(byte[] bytedata, int[] digestInt) {
    ...
    public static byte[] getDigestOfBytes(byte[] byteData) {
        int[] digestInt = new int[5];
        sha1.process_input_bytes(byteData, digestInt);
        byte[] digest = new byte[20];
        for (int i = 0; i < digestInt.length; i++) {
            sha1.intToByteArray(digestInt[i], digest, i * 4);
        }
        return digest;
    }
